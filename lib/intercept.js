...
function interceptorsFor(options) {
  ...
  _.each(allInterceptors, function(interceptor, k) {
    ...
    let body = options._form || options.body;
    if(interceptor.scopes == 0){
      isBodyMatched=true;
    } else {
      isBodyMatched = _.some(interceptor.scopes, function (scope) {
        return matches = (matchBody.call(options, scope._requestBody, body));
        // if (matches) {
        //   return true;
        // }
      });
    ...
    if (!matchingInterceptor && common.matchStringOrRegexp(basePath, interceptor.key) && isBodyMatched ) {
